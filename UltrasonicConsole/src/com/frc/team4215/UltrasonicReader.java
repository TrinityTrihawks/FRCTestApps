    ...
    public void serialEvent(SerialPortEvent event) {
		switch(event.getEventType()) {
	        ...
	        case SerialPortEvent.DATA_AVAILABLE:
		        ...
		        try {
					while (index < BufferByteSize && ((read = inputStream.read(buffer, index, BufferByteSize-index)) != -1)) {
					...
					for (int i = 0; i < index; i++) 
					{
						if (buffer[i]==82 && index-i-1 >= 4) {
							this.distance = (1000*(buffer[i+1]-48)) + (100*(buffer[i+2]-48)) + (10*(buffer[i+3]-48)) + (buffer[i+4]-48);
							break;
						}
					}			        
		        } catch (Exception e) {
